...
Using our `toolInputSchema` defined above, we'll now define the tools using the `createTool` function, which takes a description, input schema, and handler function:
...
Each tool is created with `createTool()` which takes:
- A `description` to inform the LLM about usage
- An `inputSchema` defining the expected input format
- A handler function that receives `req.arguments` with the validated input
