...
Then, let's setup the Node project with a `package.json` and the `@mokei/context-server` (server API) and `@mokei/context-server-node` (`serveProcess` stdio entry) libraries we are going to use:

<Tabs groupId="package-manager">
  <TabItem value="npm" label="npm" default>
    
    ```sh
    npm init -y && npm install @mokei/context-server @mokei/context-server-node
    ```
  ...
  <TabItem value="pnpm">
    ...
    ```sh
    pnpm init && pnpm add @mokei/context-server @mokei/context-server-node
    ```
...
```json title="package.json" {5}
{
  ...
  "dependencies": {
    ...
    "@mokei/context-server-node": "^0.1.0"
  }
}
```
...
`protocolVersions` declares the revisions this server serves. The Mokei CLI — both
`mokei inspect` and the chat session's `/context add` — defaults to `auto`, which probes the
server and speaks whichever revision it serves, so either revision alone would work for this
guide. We declare both so the server reaches clients pinned to either one, and so the
`--protocol` examples below have something to select between.
