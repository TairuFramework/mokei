...
`protocolVersions` declares both revisions, because the Mokei CLI and the chat session used
later in this guide connect at `2025-11-25` — a server declaring only `2026-07-28` would
reject them with an unsupported-protocol-version error.
...
As a sanity check, we will use the Mokei CLI to simply run the [`initialize` request](https://modelcontextprotocol.io/specification/2025-11-25/basic/lifecycle#initialization) on the server to ensure the server can initialize as expected:
