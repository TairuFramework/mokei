...
Four items left after MRTR (SEP-2322) shipped on `2026-07-28` — see
...
## 2. An empty `inputRequests` map from a foreign server burns the round cap
...
## 3. The default `requestState` minter is duplicated
...
## 4. No end-to-end test of a matched custom `mint` → echo → `verify` round trip
