...
const BANNED = [
  /^node:/,
  /^@mokei\/host-node(?:\/|$)/,
  /^@mokei\/http-client\/(?:node|oauth\/(?:file|loopback))\b/,
  /@enkaku\/node-streams/,
  /@tejika\/process/,
  /^nano-spawn$/,
]
...
function reachableSpecifiers(entry: string): { specs: Set<string>; visited: number } {
  ...
  while (stack.length) {
    ...
    for (const m of src.matchAll(/(?:from|import)\s*['"]([^'"]+)['"]/g)) {
      ...
      } else if (spec.startsWith('@mokei/') && !BANNED.some((re) => re.test(spec))) {
...
describe('Session RN bundle safety', () => {
  test('portable session reaches no Node-only specifier', () => {
    const { specs, visited } = reachableSpecifiers(entryOf('@mokei/session'))
    expect(visited).toBeGreaterThan(1)
    expect([...specs].filter((spec) => BANNED.some((re) => re.test(spec)))).toEqual([])
  })

  test('guard detects the Node session edge', () => {
    const entry = resolve(dirname(entryOf('@mokei/session')), '../../session-node/lib/index.js')
    const { specs, visited } = reachableSpecifiers(entry)
    expect(visited).toBeGreaterThan(1)
    expect([...specs].filter((spec) => BANNED.some((re) => re.test(spec)))).toContain(
      '@mokei/host-node',
    )
  })
})
